...
def reBuildEvent( connection, file, tmin=None, tmax=None, pool = None ): 
    ...
    for animal in pool.animalDictionnary.keys():
        ...
        for t in dicA.keys():
            if (dicA[t].getDistanceToPoint(xPoint = 245, yPoint = 0) == None):
                continue
            
            #Check if the animal is entering the zone around the water point:
            if (dicA[t].getDistanceToPoint(xPoint = 245, yPoint = 0) <= MAX_DISTANCE_TO_POINT*2):
            ...
            if (dicA[t].getDistanceToPoint(xPoint = 245, yPoint = 0) <= MAX_DISTANCE_TO_POINT):

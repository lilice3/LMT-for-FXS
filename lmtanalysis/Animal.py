...
# NEW FUNCTION DESIGNED 28/06/2020
    def filterDetectionSubstractionByArea(self, x1, y1, x2, y2 ):
        '''
        filter detection in the cage ( using centimeter, starting at top left of the cage )
        '''
        nbRemoved = 0
...
class AnimalPool():
    ...
    def filterDetectionSubstractionByArea(self, x1, y1, x2, y2 ):
        for animal in self.animalDictionnary.keys():
            self.animalDictionnary[animal].filterDetectionSubstractionByArea( x1, y1, x2, y2 )
    def filterDetectionByEventTimeLine(self, eventTimeLineVoc ):
        for animal in self.animalDictionnary.keys():
